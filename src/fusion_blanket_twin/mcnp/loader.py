...
class McnpMultiFieldSummary:
    ...
    def field_range(self) -> tuple[float, float]:
        return self.field_ranges[self.default_field_name]
...
def read_mcnp_multifield_summary(
    path: Path,
    default_field_name: str = REQUIRED_MCNP_FIELD,
    block_path: str = "VTKHDF/Block_2",
    required_fields: tuple[str, ...] = CANONICAL_MCNP_FIELD_ARRAYS,
) -> McnpMultiFieldSummary:
    """Read shared mesh metadata and canonical scientific field ranges."""
    h5py, np = _import_hdf_dependencies()
    if not path.exists():
        raise FileNotFoundError(f"MCNP VTKHDF file not found: {path}")
    if not path.is_file():
    ...
    with h5py.File(path, "r") as h5:
        ...
        cell_data = block["CellData"]
        cell_arrays = tuple(cell_data.keys())
        validate_required_mcnp_fields(cell_arrays)
        if default_field_name not in cell_data:
            raise ValueError(f"Default MCNP field not found: {default_field_name}")
        ...
        field_ranges = {
            field_name: (
                float(np.nanmin(cell_data[field_name][:])),
                float(np.nanmax(cell_data[field_name][:])),
            )
            for field_name in required_fields
        }

    return McnpMultiFieldSummary(
        path=path,
        block_path=block_path,
        point_count=point_count,
        cell_count=cell_count,
