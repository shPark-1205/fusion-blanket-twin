...
The optional incoming-neutron animation and plasma source are shared across Single Cell and Module modes. The animation emits a small instanced set of cyan-white streaks from the plasma-facing side of the active bounds; the source is a shallow 3D animated shader volume at that same origin. Density, speed, source visibility, and intensity are decorative presentation controls, not particle-transport or plasma-physics parameters.
